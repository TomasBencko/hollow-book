...
import { appendToHistory, startGame } from './modules/game/game.service.js';
...
export default function App() {
  ...
  const handleStart = useCallback(async (settings) => {
    ...
    try {
      const { scene, imageUrl } = await startGame(settings);
      ...
      setGameBoot({ status: 'ready', settings, scene, imageUrl, history });
    } catch (error) {
